...
#include "../zll-on-off-server_inc/af.h"
#include "../zll-on-off-server_inc/zll-on-off-server.h"
...
extern uint8_t vcMfGetTimeScaleCallback(void);
...
static EmberAfStatus writeBoolean(uint8_t endpoint, EmberAfAttributeId attribute, PGM_P name, bool value)
{
  ...
  emberAfCorePrintln("TimerOnoff:%d", value);
  if (status != EMBER_ZCL_STATUS_SUCCESS) {
...
void emberAfOnOffClusterServerTickCallback(uint8_t endpoint)
{
  ...
  //   update.
  emberAfCorePrintln("onTime:%d offWaitTime:%d", onTime, offWaitTime);
  ...
  {
	  uint8_t timeScale;
	  timeScale = vcMfGetTimeScaleCallback();
	  emberAfScheduleServerTick(endpoint,
	                              ZCL_ON_OFF_CLUSTER_ID,
								  timeScale?MILLISECOND_TICKS_PER_SECOND: MILLISECOND_TICKS_PER_SECOND / 10);
  }
}
                                               ...
                                               uint16_t offWaitTime)
{
  ...
  bool onOffAttribute;
  uint8_t endpoint = emberAfCurrentEndpoint();
  emberAfCorePrintln("%s:%d", __func__, __LINE__);
  ...
  if (onTimeAttribute < 0xFFFF && offWaitTimeAttribute < 0xFFFF) {
	  {
		  uint8_t timeScale;
		  vcReportAnAttributeEx(endpoint, ZCL_ON_OFF_CLUSTER_ID, ZCL_ON_TIME_ATTRIBUTE_ID, 0);
		  timeScale = vcMfGetTimeScaleCallback();
		  emberAfScheduleServerTick(endpoint,
		                              ZCL_ON_OFF_CLUSTER_ID,
									  timeScale?MILLISECOND_TICKS_PER_SECOND: MILLISECOND_TICKS_PER_SECOND / 10);
