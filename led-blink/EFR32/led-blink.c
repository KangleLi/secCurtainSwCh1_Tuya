...
#define MAX_BLINK_PATTERN_LENGTH  6
...
EmberEventControl emberAfPluginLedBlinkLed2EventFunctionEventControl;
EmberEventControl emberAfPluginLedBlinkLed3EventFunctionEventControl;
...
#endif
#if BSP_LED_COUNT >= 3
  & emberAfPluginLedBlinkLed2EventFunctionEventControl,
#endif
#if BSP_LED_COUNT >= 4
  & emberAfPluginLedBlinkLed3EventFunctionEventControl,
#endif
};
...
#if BSP_LED_COUNT == 4
static gpioBlinkState ledEventState[BSP_LED_COUNT] = { LED_ON, LED_ON, LED_ON, LED_ON};
static uint16_t ledBlinkCount[BSP_LED_COUNT] = { 0x00, 0x00,0x00, 0x00};
static uint8_t activeLed[BSP_LED_COUNT] = HAL_LED_ENABLE;
#elif BSP_LED_COUNT == 2
...
// Plugin led2 event handler
void emberAfPluginLedBlinkLed2EventFunctionEventHandler(void)
{
  handleLedEvent(2);
}

// Plugin led3 event handler
void emberAfPluginLedBlinkLed3EventFunctionEventHandler(void)
{
  handleLedEvent(3);
}
    ...
    switch (ledEventState[ledIndex]) {
      ...
      case LED_BLINK_PATTERN:
        if (ledBlinkCount[ledIndex] == 0) {
          ...
          vcLedBlinkFinishedCallback(activeLed[ledIndex], blinkPatternIndex[ledIndex]);
          break;
        }

        if (ledBlinkCount[ledIndex] % 2 == 0) {
          turnLedOff(activeLed[ledIndex]);
        } else {
          turnLedOn(activeLed[ledIndex]);
        }
        ...
        //  blinkPatternIndex[ledIndex] = 0;
          if (ledBlinkCount[ledIndex] != 0xFFFF) { // blink forever if count is 255
            ledBlinkCount[ledIndex]--;
          }
        //}
        break;
...
void halMultiLedBlinkPatternInit(uint8_t  length,
                             uint16_t *pattern,
                             uint8_t  led)
{
  uint8_t i, ledIndex;
...
void halMultiLedBlinkPatternActivity(uint8_t  led,
                             uint8_t  pattern,
							 uint16_t count)
{
  uint8_t i, ledIndex;
  ...
  ledBlinkCount[ledIndex] = count;
  blinkPatternIndex[ledIndex] = pattern;
  emberEventControlSetDelayMS(*(ledEventArray[ledIndex]),
