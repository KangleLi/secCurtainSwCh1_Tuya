...
static uint8_t buttonState[BSP_BUTTON_COUNT] = {BUTTON_RELEASED, BUTTON_RELEASED, BUTTON_RELEASED, BUTTON_RELEASED};
...
void halInternalButtonIsr(uint8_t pin)
{
  ...
  uint32_t iflags = 0x01;
  #if (DEBOUNCE > 0)
  uint8_t buttonStatePrev;
  uint32_t debounce;
  uint32_t debounceTotal;
  #endif //(DEBOUNCE > 0)
  iflags <<= pin;
  GPIO_IntDisable(iflags);

  buttonStateNow = halButtonPinState(pin);
  #if (DEBOUNCE > 0)
  //read button until get "DEBOUNCE" number of consistent readings
  for ( debounce = debounceTotal = 0;
        debounce < DEBOUNCE && debounce < debounceTotal;
...
void vcHalButtonSetISR(uint8_t pin)
{
    /* Register callbacks before setting up and enabling pin interrupt. */
    GPIOINT_CallbackRegister(pin,
                             halInternalButtonIsr);
}
